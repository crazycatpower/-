...
BASE_SYSTEM_PROMPT = (
    ...
    "【分工】YOLO 偵測清單只是輔助參考，代表 YOLO 模型「可能」看到的物件與位置，不是絕對正確的事實——"
    "YOLO 常常誤判（尤其低信心的框，或訓練資料裡沒見過的物件類型），你的判斷應該以自己對原圖（與標註圖）的"
    "實際觀察為主；清單可以提示你去圖片哪個座標範圍仔細看，但看了之後如果原圖內容跟清單描述兜不起來，"
    "以你自己看到的實際畫面內容為準，不要因為清單有列出某個類別就照單全收、也不要只因清單沒列出某物件"
    "就假設它不存在。圖片本身才是主要依據，清單只是輔助線索。\n"
    ...
    "（如《職業安全衛生法》第幾條、《營造安全衛生設施標準》第幾條）。"
    "【參考法規】片段若有提供，每個片段開頭都會標示該條文實際的條號（如「第46條」），"
    "務必直接照抄那個條號，不可以只寫法規名稱卻不寫條號、也不可以自己編一個條號；"
    "只有在【參考法規】完全沒有提供相關片段時，才能只寫法規名稱或使用一般性建議。"
    "絕對不可以把本系統自己的稽核表項次代碼（如「1.02」「10.02」「環保-3」）當成"
...
def format_detections_for_prompt(detections: list[dict]) -> str:
    ...
    header = "[YOLO 偵測清單 — 僅供輔助參考，請以你對原圖的實際觀察為準逐項核實]"
    if low_conf_count:
        header += (
